...
Reads `.planning/ROADMAP.md` in the current checkout or a dedicated claptrap milestone worktree, determines the active milestone and phase state, presents 1–3 logical next actions, and triggers the appropriate claptrap sub-skill.

## Algorithm

1. Resolve the workflow workspace root:
   - If `.planning/ROADMAP.md` exists in the current checkout, use the current checkout.
   - Otherwise look for `.worktrees/*/.planning/ROADMAP.md`.
   - If none exist, treat status as `Not Started`.
   - If exactly one exists, use that worktree as the workspace root.
   - If multiple exist, ask the user which workspace to inspect before continuing.
...
3. If status is `In progress`, run `git status` in the resolved workspace root to check for unmerged local changes. Record whether the workspace is **dirty** or **clean**.
...
- **Using the wrong workspace:** If the milestone was started in `.worktrees/M##-slug/`, keep using that workspace for plan, execute, and completion work until the milestone is done.
